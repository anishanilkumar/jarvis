{ config, lib, ... }:
{
  ...
  # App to launch YouTube Music with the account signed in on it.
  #
  # Reachable on the private network only — this is an admin surface, not
  # something the household or LAN guests should reach.
  #
  # RAM note: HA wants roughly 0.5-1 GB. On a Pi 4 already running the wall, a
  # media server, an X desktop and noVNC, check `free -h` before and after
  # enabling. If it is tight, cut `extraComponents` further rather than reaching
  # for swap — swap on the SSD is what historically caused the under-voltage
  # dips (see an under-voltage logger).
